public class AVL {
    ...
    private int naoFolhasRecursivo(No noAtual){
        ...
        if(noAtual.esquerdo != null || noAtual.direito != null){
                     return 1 + naoFolhasRecursivo(noAtual.direito) +
                     naoFolhasRecursivo(noAtual.esquerdo);
        }
        return 0; //é folha, não conta
    }

    //verificar se a arvore é estritamente binaria
    public boolean estritamenteBinaria(){ //0 ou 2 filhos
    ...
    private boolean estritamenteBinariaRecursivo(No noAtual){
        ...
        return estritamenteBinariaRecursivo(noAtual.esquerdo) && estritamenteBinariaRecursivo(noAtual.direito);
    }
    ...
    private boolean completaRecursiva(No no, int indice, int totalNos){
        ...
        return completaRecursiva(no.esquerdo, 2* indice + 1, totalNos) &&
        completaRecursiva(no.direito, 2* indice+2, totalNos);
    }
    ...
    private boolean cheiaRecursivo(No no){
        if(no == null){
            return true; //arvore vazia é cheia
        }
    ...
    private boolean ehAVLRecursivo(No no){
        if(no == null){
            return true;
        }
        ...
        return ehAVLRecursivo(no.esquerdo) && ehAVLRecursivo(no.direito);
    }
}